...
function upgrade30_dbchanges()
{
	...
	echo "<p>اجرای دستورهای لازم به‌روزرسانی...</p>";
	flush();
	...
	$output->print_contents("<p>برای رفتن به مرحله‌ی بعدی عملیات به‌روز‌رسانی، بر روی \"مرحله‌ی بعد\" کلیک کنید.</p>");
...
function upgrade30_dbchanges6()
{
	...
	echo "<p>به‌روزرسانی دسترسی‌های گروه‌ها...</p>";
	$groups = array();
...
function upgrade30_threadcount()
{
	...
	$output->print_header("شمارش تعداد موضوعات کاربر");
	...
	echo "<p>شمارش تعداد موضوعات کاربران از کاربر #{$lower} تا کاربر #{$upper} (درکل {$cnt['usercount']})</p>";
	flush();
	...
	if($remaining && $threadnum)
	{
		...
		$contents = "<p><input type=\"hidden\" name=\"theadspage\" value=\"$threads\" /><input type=\"hidden\" name=\"threadstart\" value=\"$startat\" />انجام شد. برای شمارش تعداد موضوعات مابقی کاربران روی \"مرحله‌ی بعد\" کلیک کنید.</p>";
	}
	else
	{
		$nextact = "30_dbchanges_optimize1";
		$contents = "<p>انجام شد</p><p>شمارش تعداد موضوعات تمامی کاربران انچام شد. برای رفتن به مرحله‌ی بعدی عملیات به‌روز‌رسانی، بر روی \"مرحله‌ی بعد\" کلیک کنید.</p>";
	}
...
function upgrade30_dbchanges_optimize1()
{
	...
	$output->print_header("بهینه‌سازی بانک اطلاعاتی");

	echo "<p>اجرای دستورهای لازم بهینه‌سازی...</p>";
	flush();
...
function upgrade30_dbchanges_optimize2()
{
	...
	echo "<p>اجرای دستورهای لازم بهینه‌سازی...</p>";
	echo "<p>اضافه کردن شاخص‌ها به جداول...</p>";
	flush();
	...
	echo "<p>حذف شاخص‌های قدیمی از جداول...</p>";
...
function upgrade30_dbchanges_optimize3()
{
	...
	{
		echo "<p>{$table}: تبدیل نوع ستون به رشته‌ی کوچک</p>";
		$change_column = array();
...
function upgrade30_dbchanges_optimize4()
{
	...
	{
		echo "<p>{$table}: تبدیل نوع ستون به رشته</p>";
		$change_column = array();
		foreach($columns as $column)
		{
			if($db->type == "pgsql")
...
function upgrade30_dbchanges_smilies()
{
	...
	$output->print_header("به‌روزرسانی شکلک‌ها");
...
function upgrade30_dbchanges_ip()
{
	...
	$output->print_header("تبدیل IPها");
	...
	{
		case 8:
			echo "<p>افزودن شاخص‌های بانک اطلاعاتی (3/3)...</p>";
			flush();
			...
			break;
		case 7:
			echo "<p>افزودن شاخص‌های بانک اطلاعاتی (2/3)...</p>";
			flush();
			...
			break;
		case 6:
			echo "<p>افزودن شاخص‌های بانک اطلاعاتی (1/3)...</p>";
			flush();
		...
		case 5:
			...
			echo "<p>تبدیل IPها از {$lower} تا {$upper} (درکل {$cnt['ipcount']})</p>";
			flush();
		...
		case 4:
			...
			{
				case 10:
					echo "<p>به‌روزرسانی جدول کاربران (4/4)...</p>";
					flush();
					...
					break;
				case 9:
					echo "<p>به‌روزرسانی جدول کاربران (3/4)...</p>";
					flush();
					...
					break;
				case 8:
					echo "<p>به‌روزرسانی جدول امتیازات موضوع‌ها...</p>";
					flush();
					...
					break;
				case 7:
					echo "<p>به‌روزرسانی جدول جلسه‌ها...</p>";
					flush();
					...
					break;
				case 6:
					echo "<p>به‌روزرسانی جدول جستجوهای ثبت شده...</p>";
					flush();
					...
					break;
				case 5:
					echo "<p>به‌روزرسانی جدول ارسال‌ها (2/2)...</p>";
					flush();
					...
					break;
				case 4:
					echo "<p>به‌روزرسانی جدول کارهای ثبت شده‌ی ناظم...</p>";
					flush();
					...
					break;
				case 3:
					echo "<p>به‌روزرسانی جدول ایمیل‌های ثبت شده...</p>";
					flush();
					...
					break;
				case 2:
					echo "<p>به‌روزرسانی جدول جلسه‌های مدیرکل...</p>";
					flush();
					...
					break;
				default:
					echo "<p>به‌روزرسانی جدول کارهای ثبت شده‌ی مدیرکل...</p>";
					flush();
			...
			break;
		case 3:
			echo "<p>به‌روزرسانی جدول کاربران (2/4)...</p>";
			flush();
			...
			break;
		case 2:
			echo "<p>به‌روزرسانی جدول کاربران (1/4)...</p>";
			flush();
			...
			break;
		default:
			echo "<p>به‌روزرسانی جدول ارسال‌ها (1/2)...</p>";
			flush();
	...
	if($next_task == 9)
	{
		$contents = "<p>برای رفتن به مرحله‌ی بعدی عملیات به‌روز‌رسانی، بر روی \"مرحله‌ی بعد\" کلیک کنید.</p>";
		$nextact = "30_updatetheme";
	}
	else
	{
		$contents = "<p><input type=\"hidden\" name=\"iptask\" value=\"{$next_task}\" />{$iptable}{$ipstart}انجام شد. برای ادامه‌ی فرآیند تبدیل IPها،  بر روی \"مرحله‌ی بعد\" کلیک کنید</p>";
...
function upgrade30_updatetheme()
{
	...
	{
		$output->print_error("لطفاً مطمئن شوید که مسیر فایل‌های مربوط به بخش کنترل پنل مدیرکل (admin) به درستی آپلود شده است.");
	}

	$output->print_header("به‌روزرسانی پوسته‌ها");

	// New default user star
	$contents = "<p>به‌روزرسانی تصویر پیش‌فرض ستاره‌های کاربر... ";
	$db->update_query("usergroups", array('starimage' => 'images/star.png'), "starimage='images/star.gif'");
	$contents .= "انجام شد.</p>";

	$contents .= "<p>افزودن قالب بندی‌های جدید... ";
	...
	$contents .= "<p>افزودن چیدمان نمایش به قالب بندی‌ها... ";
	...
	$contents .= "<p>افزودن رنگ‌های پیش‌فرض... ";
	...
	$contents .= '<p>بازسازی کَش و کوچک کردن شیوه‌نامه‌های موجود...</p>';

	$num_re_cached = recache_existing_styles();

	$contents .= "انجام شد. بازسازی کَش {$num_re_cached} قالب بندی با موفقیت انجام شد.";
	...
	{
		$output->print_footer("30_انجام شد");
	}
}
...
function upgrade30_acppin()
{
	...
	$output->print_header("افزودن کد امنیتی به کنترل پنل مدیرکل");

	echo "<p>ما یک قابلیت امنیتی جدید را در نسخه‌ی 1.8 قرار داده‌ایم! قابلیت تعیین کد امنیتی برای ورود به کنترل پنل مدیرکل.<br />\n";
	echo "اگر نمی‌خواهید برای ورود به کنترل پنل مدیرکل کد امنیتی قرار دهید، می‌توانید با خالی گذاشتن جعبه‌ی زیر، این مرحله را رد کنید. شما می‌توانید بعداً این قابلیت را فعال کنید. (برای اطلاع از نحوه‌ی فعال کردن این قابلیت در آینده، به صفحه‌ی مستندات مراجعه کنید)</p>\n";
	echo '<div class="border_wrapper">
			<div class="title">تنظیمات کد امنیتی ورود به کنترل پنل مدیرکل</div>
			<table class="general" cellspacing="0">
				<tbody>
				<tr>
					<th colspan="2" class="first last">کد امنیتی کنترل پنل مدیرکل</th>
				</tr>
				<tr class="first">
					<td class="first"><label for="bbname">کد امنیتی:</label></td>
...
function upgrade30_acppin_submit()
{
	...
	$content = "<p>اکنون در حال نوشتن کد امنیتی (در صورتی که مقداری برای آن وارد کرده باشید) در فایل پیکربندی هستیم... ";

	if(!is_writable(MYBB_ROOT."inc/config.php"))
	{
		$content .= "ناموفق (فایل config.php قابل نوشتن نیست)";
	}
	else if(isset($config['secret_pin']))
	{
		$content .= "رد شد (کد امنیتی قبلاً تعیین شده است)";
	}
	else
	{
