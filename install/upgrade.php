...
else
{
	...
	else if($mybb->input['action'] == "do_login" && $mybb->request_method == "post")
	{
		...
		{
			$output->print_error("نام کاربری وارد شده صحیح نمی‌باشد.");
		}
		$options = array(
		...
		else
		{
			...
			if(!$user['uid'])
			{
				$output->print_error("گذرواژه‌ی وارد شده صحیح نمی‌باشد. درصورتی که گذرواژه‌ی خود را فراموش کرده‌اید، <a href=\"../member.php?action=lostpw\">اینجا</a> را کلیک کنید. در غیر این‌صورت، بازگردید و مجدداً تلاش کنید.");
			}
		}
...
function buildsettings()
{
	...
	{
		$output->print_header("بازسازی تنظیمات");
		echo "<p><div class=\"error\"><span style=\"color: red; font-weight: bold;\">خطا: امکان خواندن فایل inc/settings.php وجود ندارد.</span><h3>به منظور ادامه‌ی عملیات به‌روزرسانی، فایل inc/settings.php را قابل خواندن و نوشتن کنید.</h3></div></p>";
		$output->print_footer("rebuildsettings");
		exit;
	}
...
function upgradedone()
{
	...
	ob_start();
	$output->print_header("به‌روزرسانی به پایان رسید");
